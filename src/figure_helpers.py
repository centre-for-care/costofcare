...
def plot_EDA_figure(result, df):
    ...
    grouped_control = pd.DataFrame(
        df[df['care_intensity_t'].isnull()].groupby(['dvage'])[
            'ind_inc_deflated'].mean()).reset_index()
    grouped_low = pd.DataFrame(
        df[(df['care_intensity_t'] == '0-4 hours')].groupby(['dvage'])[
                                   'ind_inc_deflated'].mean()).reset_index()
    grouped_medlow = pd.DataFrame(
        df[(df['care_intensity_t'] == '5-19 hours')].groupby(['dvage'])[
            'ind_inc_deflated'].mean()).reset_index()
    grouped_medhigh = pd.DataFrame(
        df[(df['care_intensity_t'] == '20-49 hours')].groupby(['dvage'])[
            'ind_inc_deflated'].mean()).reset_index()
    grouped_high = pd.DataFrame(
        df[(df['care_intensity_t'] == '50+hours')].groupby(['dvage'])[
            'ind_inc_deflated'].mean()).reset_index()

    lowess_smoothed_control = lowess(grouped_control['ind_inc_deflated'], grouped_control['dvage'], frac=0.3)
    ...
    smoothed_control = pd.DataFrame(lowess_smoothed_control, columns=['dvage', 'ind_inc_deflated_smoothed'])
    smoothed_control.set_index('dvage').plot(ax=ax3, color='k', legend=False)
    ...
    grouped_df = grouped_df.reindex(['0-4 hours', '5-19 hours', '20-49 hours', '50+hours'])
    ...
    ax4.set_xticklabels(['0-4 Hours', '5-19 Hours', '20-49 Hours', '50+ Hours'])
    legend_elements4 = [
        Patch(facecolor=colors[0], edgecolor=(0, 0, 0, 1),
              label=r'Female'),
        Patch(facecolor=colors[3], edgecolor=(0, 0, 0, 1),
    ...
    ax5.set_xticklabels(['0-4 Hours', '5-19 Hours', '20-49 Hours', '50+ Hours'])
    legend_elements5 = [
        Patch(facecolor=colors[0], edgecolor=(0, 0, 0, 1),
              label=r'White'),
        Patch(facecolor=colors[3], edgecolor=(0, 0, 0, 1),
...
def make_weighted_intensity_table(df):
    ...
    result = result[['0-4 hours', '5-19 hours', '20-49 hours', '50+hours']]
